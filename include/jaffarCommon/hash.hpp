 ...
 */
__JAFFAR_COMMON_INLINE__ std::string getSHA1String(const std::string& string) { return sha1::SHA1::GetHash((const uint8_t*)string.data(), string.size()); }
 ...
 */
__JAFFAR_COMMON_INLINE__ hash_t calculateMetroHash(const void* data, size_t size)
{
  MetroHash128 hash;
 ...
 */
__JAFFAR_COMMON_INLINE__ std::string hashToString(const hash_t hash)
{
 ...
 */
__JAFFAR_COMMON_INLINE__ hash_t hashString(const std::string& string) { return calculateMetroHash(string.data(), string.size()); }
