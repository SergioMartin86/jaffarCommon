...
class Differential final : public deserializer::Base
{
   ...
   */
  Differential(const void *__restrict inputDataBuffer     = nullptr,
  ...
  __INLINE__ void popContiguous(void *const __restrict outputDataBuffer, const size_t outputDataSize) override
  {
    ...
    if (_inputDataBufferPos + outputDataSize > _inputDataBufferSize)
      JAFFAR_THROW_RUNTIME(
        "Maximum input data position reached before contiguous deserialization of (%lu + %lu > %lu) bytes", _inputDataBufferPos, outputDataSize, _inputDataBufferSize);
    if (_referenceDataBufferPos + outputDataSize > _referenceDataBufferSize)
      JAFFAR_THROW_RUNTIME("[Error] Maximum reference data position to be exceeded on contiguous deserialization (%lu + %lu > %lu)",
                           _referenceDataBufferPos,
                           outputDataSize,
  ...
  __INLINE__ void pop(void *const __restrict outputDataBuffer, const size_t outputDataSize) override
  {
    ...
    if (_inputDataBufferPos + differentialBufferSize >= _inputDataBufferSize)
      JAFFAR_THROW_RUNTIME(
        "[Error] Maximum input data position reached before differential buffer size decode (%lu + %lu > %lu)", _inputDataBufferPos, differentialBufferSize, _inputDataBufferSize);
    ...
    if (_referenceDataBufferPos + outputDataSize > _referenceDataBufferSize)
      JAFFAR_THROW_RUNTIME(
        "[Error] Maximum reference data position exceeded before differential decode (%lu + %lu > %lu)", _referenceDataBufferPos, outputDataSize, _referenceDataBufferSize);

    // Encoding differential
    usize_t output_size;
    int     ret = xd3_decode_memory(&_inputDataBuffer[_inputDataBufferPos],
                                diffCount,
