 ...
 */
inline constexpr uint8_t bitMaskTable[8] = {0b00000001, 0b00000010, 0b00000100, 0b00001000, 0b00010000, 0b00100000, 0b01000000, 0b10000000};
 ...
 */
inline constexpr uint8_t bitNotMaskTable[8] = {0b11111110, 0b11111101, 0b11111011, 0b11110111, 0b11101111, 0b11011111, 0b10111111, 0b01111111};
                                       ...
                                       const size_t srcBufferOffset, const size_t count, const size_t elementBitSize)
{
  ...
  // Offsets are expressed in elements, so the actual bit position is offset * elementBitSize.
  // The overflow guards must be computed in bits to correctly catch out-of-bounds accesses.
  ...
  if (dstOffsetBits + totalBitCount > dstBufferSizeBits)
    JAFFAR_THROW_LOGIC("The operation will overflow destination buffer (%lu + %lu > %lu)", dstOffsetBits, totalBitCount, dstBufferSizeBits);
 ...
 * @return The number of bits required. Examples: (Element Count, Bits Required) => (0,0), (1,1), (2,1), (3,2), (4,2), (5,3), (6,3), (7,3), (8,3), (9,4)
 * @note A single element is given 1 bit (not 0): downstream storage (e.g. bit-packed buffers and bitcopy) requires a positive element bit size.
 */
