...
template <typename T>
__INLINE__ const std::vector<T> getArray(const nlohmann::json &json, const std::string &entry)
{
  checkEntry(json, entry);
...
template <typename T>
__INLINE__ const T getNumber(const nlohmann::json &json, const std::string &entry)
{
  checkEntry(json, entry);
